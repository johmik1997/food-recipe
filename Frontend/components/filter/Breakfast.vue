...
onMounted(async () => {
  try {
    ...
    await recipesStore.filterByCategory("Breakfast");
  } catch (error) {
...
<template>
  <div>
    <div v-if="recipesStore.recipes.length === 0">
      <div class="flex flex-col items-center gap-4 p-6 bg-amber-50">
        <h1 class="text-center font-poppins-italic text-2xl md:text-4xl font-bold text-amber-800">
          No Breakfast recipes found, be the first to create one!
        </h1>
        <NuxtLink to="/recipes/create" class="relative group cursor-pointer">
          ...
          />
          <div
            class="absolute inset-0 bg-amber-500 bg-opacity-70 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          >
    ...
    <div v-else class="relative font-poppins bg-amber-50">
      <!-- Left Chevron Button -->
      <button
        @click="scrollCarousel('left')"
        class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-amber-500 text-white p-3 rounded-full shadow-lg hover:bg-amber-600 transition-colors duration-200 z-10"
      >
        <svg
      ...
      <div
        ref="carouselRef"
        class="flex overflow-x-auto scroll-smooth snap-x snap-mandatory gap-6 p-4 transition-transform duration-1000 ease-in-out bg-amber-50"
        style="scrollbar-width: none; -ms-overflow-style: none"
      >
        <div
          v-for="recipe in recipesStore.recipes"
          :key="recipe.id"
          class="flex flex-col bg-white w-96 border-2 border-amber-100 rounded-xl overflow-hidden flex-shrink-0 snap-center transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
        >
          <figure class="relative h-60 overflow-hidden">
            ...
            >
              <svg
                ...
                class="h-6 w-6"
                :class="{
                  'text-amber-500 fill-amber-500': bookmarkStates[recipe.id],
                  'text-amber-300': !bookmarkStates[recipe.id],
                }"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
          ...
          <div class="p-6 flex flex-col flex-1">
            <NuxtLink :to="`/recipes/${recipe.id}`" class="group">
              <h2 class="text-xl font-bold text-amber-800 mb-2 group-hover:text-amber-600 transition-colors">
                ...
                <div class="flex mr-2">
                  <span
                    ...
                    :class="{
                      ...
                      'text-amber-100': star > Math.round(recipe.average_rating),
                    }"
                  >
                    ★
                  </span>
            ...
            <div class="flex justify-between items-center mt-4">
              <span class="text-amber-700 bg-amber-100 px-3 py-1 rounded-full text-sm font-medium">
      ...
      <button
        ...
        class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-amber-500 text-white p-3 rounded-full shadow-lg hover:bg-amber-600 transition-colors duration-200 z-10"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-6 w-6"
