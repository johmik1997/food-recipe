...
onMounted(async () => {
  try {
    ...
    await recipesStore.filterByCategory("Lunch");
  } catch (error) {
  ...
  <div>
    <div v-if="recipesStore.recipes.length === 0">
      <div class="flex flex-col items-center gap-4 p-6 bg-white">
        <h1
          class="text-center font-poppins-italic text-2xl md:text-4xl font-bold text-gray-800"
        >
          No Lunch recipes found, be the first to create one!
        </h1>
